...
# SimpleModelWrapper and GeneralModelWrapper were modified to support *args, **kwargs (multiple inputs and flags for model inference)
...
class SimpleModelWrapper(ModelWrapper):
    ...
    def forward(self, *x, enc=False, **kwargs):
        if enc:
            return self.modules[0].encoder(*x, **kwargs)
        else:
            return self.modules[0](*x, **kwargs)
