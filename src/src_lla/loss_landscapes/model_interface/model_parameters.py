...
# Incorporated into loss landscape analysis (lla) library mostly without modifications 
# Some functions were corrected according to issue solutions suggested in the original repo
# weight_normalize_ method was implemented to allow normalization='weight'
...
class ModelParameters:
    ...
    '''
    #'''
    # git issues implementation fixing multiple applications
    def filter_normalize_(self, ref_point: 'ModelParameters', order=2):
        """
        In-place filter-wise normalization of the tensor.
        :param ref_point: use this model's filter norms, if given
    ...
    def weight_normalize_(self, ref_point: 'ModelParameters', order=None):
        """
        In-place weight-wise normalization of the tensor.
        Simple multiplication of dir params by weights
        Original implementation:  https://github.com/tomgoldstein/loss-landscape/blob/master/net_plotter.py#L98

        :ref_point: original model's weights
        :return: none 
        """
        for l in range(len(self.parameters)):
            # normalize one-dimensional bias vectors
            if len(self.parameters[l].size()) == 1:
                self.parameters[l] *= ref_point.parameters[l]
            else:
                # normalize two-dimensional weight vectors
                for f in range(len(self.parameters[l])):
                    self.parameters[l][f] *= ref_point.parameters[l][f]
    
    def weight_norm(self, order=None)  -> float:
        """
...
def orthogonal_to(vector: ModelParameters,u_like=True) -> ModelParameters:
    """
    Create a new ModelParameters object of size and shape compatible with the given
    ...
    """
    new_vector = rand_u_like(vector) if u_like else rand_n_like(vector)
